...
function Login() {
  ...
  async function Submit() {
    ...
    try {
      const response = await axios({
        url: "https://finalproject-helpmesell.herokuapp.com/userslogin/",
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
      ...
      if (response.data.token) {
        const { token, first_name, username } = response.data || {};
        let userObj = {
          token,
          first_name,
          username,
        };
        localStorage.setItem("current_user", userObj);
        history("/homepage");
      }
  ...
  return (
    <div>
      ...
      <Button onClick={AdminSubmit} className={classes.loginAdmin}>
